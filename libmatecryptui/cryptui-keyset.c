...
cryptui_keyset_constructor (GType type, guint n_props, GObjectConstructParam* props)
{
    ...
    path = g_strdup_printf("/org/mate/bastile/keys/%s", keyset->priv->keytype);

    keyset->priv->remote_keyset = dbus_g_proxy_new_for_name (bus,
                    "org.mate.bastile", path, "org.mate.bastile.Keys");
    keyset->priv->remote_service = dbus_g_proxy_new_for_name (bus,
                    "org.mate.bastile", "/org/mate/bastile/keys", "org.mate.bastile.KeyService");
...
/**
 * cryptui_keyset_new:
 * @keytype: libmatecryptui key prefix
 ...
 * @keyset: a libmatecryptui keyset
 *
 * Gets the libmatecryptui key prefix for the keyset
 *
 * Returns: a libmatecryptui key prefix
 */
const gchar*
 ...
 * @keyset: a libmatecryptui keyset
 * @key: libmatecryptui key to cache
 *
 ...
 * @keyset: a libmatecryptui keyset
 * @key: libmatecryptui key to fetch a property of
 ...
 * @keyset: a libmatecryptui keyset
 * @keys: an array of libmatecryptui keys
 *
