...
load_sort_order (CryptUIKeyStore *ckstore)
{
    ...
    if (ckstore->priv->sortable)
        sort = _cryptui_mateconf_get_string (BASTILE_RECIPIENTS_SORT_KEY);
...
sort_changed (GtkTreeSortable *sort, CryptUIKeyStore *ckstore)
{
    ...
    _cryptui_mateconf_set_string (BASTILE_RECIPIENTS_SORT_KEY, x);
    g_free (x);
}
 ...
 * @keyset: a libmatecryptui keyset
 * @use_checks: a gboolean
 * @none_option: a const gchar string
 *
 * Creates a new libmatecryptui key store setting the keystore, use-checks and none-option
 * properties
 *
 * Returns: the new libmatecryptui key store
 */
CryptUIKeyStore*
 ...
 * @ckstore: a libmatecryptui key store
 *
 * Gets ckstore's internal keyset
 *
 * Returns: the internal libmatecryptui keyset
 */
CryptUIKeyset*
 ...
 *
 * Returns: a list of libmatecryptui keys
 */
GList*
 ...
 * @ckstore: a libmatecryptui key store
 * @view: a GtkTreeView
 * @keys: a list of libmatecryptui keys
 *
