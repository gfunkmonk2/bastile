...
init_remote_service ()
{
    ...
    remote_service = dbus_g_proxy_new_for_name (bus, "org.mate.bastile", 
                              "/org/mate/bastile/keys", "org.mate.bastile.KeyService");
 ...
 * @key: (in): key for use with libmatecryptui
 *
 ...
 * key details are discovered.  See http://live.mate.org/Bastile/DBus for a
 ...
 *
 * This function is called when bastile needs to be launched to generate a
...
/**
 * _cryptui_mateconf_get_boolean:
 * @key: a mateconf key/path
 *
 * A private library function that gets a boolean from a mateconf key.
 *
 * Returns: The boolean stored at the key location or FALSE if an error
 *          occured.
 */
gboolean
_cryptui_mateconf_get_boolean (const char *key)
{
    MateConfClient *client = get_global_client ();
    gboolean result;
    GError *error = NULL;
...
/**
 * _cryptui_mateconf_get_string:
 * @key: a mateconf key/path
 *
 * A private library function that gets a string from a mateconf key.
 *
 * Returns: The string stored at the key location or the null string if an error
 *          occured.
 */
char *
_cryptui_mateconf_get_string (const char *key)
{
    char *result;
...
/**
 * _cryptui_mateconf_set_string:
 * @key: a mateconf key/path
 * @string_value: a text string
 *
 * A private library function that sets a mateconf key to the string given.
 */
void
_cryptui_mateconf_set_string (const char *key, const char *string_value)
{
    MateConfClient *client = get_global_client ();
    GError *error = NULL;
...
/**
 * _cryptui_mateconf_notify:
 * @key: a mateconf key/path
 * @notification_callback: function to be called by the notification
 * @callback_data: data to be passed to the callback function
 *
 * A private library convenience function that creates a mateconf notification
 * on a specified mateconf key.
 *
 * Returns: the mateconf notification id
 */
guint
_cryptui_mateconf_notify (const char *key, MateConfClientNotifyFunc notification_callback,
...
internal_mateconf_unnotify (gpointer data)
{
    ...
    _cryptui_mateconf_unnotify (notify_id);
}

/**
 * _cryptui_mateconf_notify_lazy:
 * @key: a mateconf key/path
 * @notification_callback: function to be called by the notification
 * @callback_data: data to be passed to the callback function
 ...
 *
 * A private library convenience function that creates a mateconf notification
 * on a specified mateconf key and automatically removes the notification when an
 * object is destroyed.
 */
void
_cryptui_mateconf_notify_lazy (const char *key, MateConfClientNotifyFunc notification_callback,
                            gpointer callback_data, gpointer lifetime)
{
    guint notification_id;
    gchar *t;
    
    notification_id = _cryptui_mateconf_notify (key, notification_callback, callback_data);
    if (notification_id != 0) {
        t = g_strdup_printf ("_cryutui-mateconf-notify-lazy-%d", notification_id);
...
/**
 * _crytpui_mateconf_unnotify:
 * @notification_id: a current mateconf notification id
 *
 * A private library function to remove a mateconf notification.
 */
void
_cryptui_mateconf_unnotify (guint notification_id)
{
    MateConfClient *client = get_global_client ();
    g_return_if_fail (client != NULL);
