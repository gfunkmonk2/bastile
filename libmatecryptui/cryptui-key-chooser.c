...
{
    gchar *mateconf, *value;
    g_return_val_if_fail (keyset, NULL);
    
    mateconf = g_strconcat (key, "_", cryptui_keyset_get_keytype (keyset), NULL);
    value = _cryptui_mateconf_get_string (mateconf);
    g_free (mateconf);
...
set_keyset_value (CryptUIKeyset *keyset, const gchar *key, const gchar *value)
{
    ...
    mateconf = g_strconcat (key, "_", cryptui_keyset_get_keytype (keyset), NULL);
    _cryptui_mateconf_set_string (mateconf, value ? value : "");
    g_free (mateconf);
}
...
cryptui_key_chooser_get_recipients (CryptUIKeyChooser *chooser)
{
    ...
    if (!chooser->priv->enforce_prefs || 
        !_cryptui_mateconf_get_boolean (BASTILE_ENCRYPTSELF_KEY))
        return recipients;
    ...
    if (key == NULL) {
        gchar *gval = get_keyset_value (keyset, BASTILE_DEFAULT_KEY);
        if (gval != NULL)
