...
static char* 
bastile_pkcs11_klass_to_string (gulong klass) 
{
	switch (klass)
	{
	case CKO_DATA:
...
static gulong 
bastile_pkcs11_string_to_klass (const char* str) 
{
	gulong klass;
	gchar *end;
...
GQuark 
bastile_pkcs11_id_from_attributes (GckAttributes* attrs)
{
	gulong klass;
	GckAttribute* attr;
	...
	klass_str = bastile_pkcs11_klass_to_string (klass);
	id = g_strdup_printf ("%s:%s/%s", BASTILE_PKCS11_TYPE_STR, klass_str, encoded);
	g_free (encoded);
	g_free (klass_str);
...
gboolean 
bastile_pkcs11_id_to_attributes (GQuark id, GckAttributes* attrs)
{
	const gchar* value;
	guchar *ckid;
	gsize n_ckid;
	...
	if (!parts[0] || !g_str_equal (parts[0], BASTILE_PKCS11_TYPE_STR) ||
	    !parts[1] || !parts[2]) {
		g_strfreev (parts);
		return FALSE;
	}
	
	klass = bastile_pkcs11_string_to_klass (parts[1]);
	if (klass == -1) {
		g_strfreev (parts);
		return FALSE;
	}
