...
#include "bastile-registry.h"

typedef struct _BastileRegistryPrivate BastileRegistryPrivate;

struct _BastileRegistryPrivate {
	GHashTable *types;
	GHashTable *objects;
	GHashTable *functions;
};

#define BASTILE_REGISTRY_GET_PRIVATE(o) \
	(G_TYPE_INSTANCE_GET_PRIVATE((o), BASTILE_TYPE_REGISTRY, BastileRegistryPrivate))

G_DEFINE_TYPE(BastileRegistry, bastile_registry, G_TYPE_OBJECT);

static BastileRegistry *registry_singleton = NULL; 
...
static void
bastile_registry_init (BastileRegistry *obj)
{
	BastileRegistryPrivate *pv = BASTILE_REGISTRY_GET_PRIVATE (obj);
...
static void
bastile_registry_dispose (GObject *obj)
{
	BastileRegistryPrivate *pv = BASTILE_REGISTRY_GET_PRIVATE (obj);
	g_hash_table_remove_all (pv->types);
	G_OBJECT_CLASS (bastile_registry_parent_class)->dispose (obj);
}

static void
bastile_registry_finalize (GObject *obj)
{
	BastileRegistryPrivate *pv = BASTILE_REGISTRY_GET_PRIVATE (obj);
	g_hash_table_destroy (pv->types);
	G_OBJECT_CLASS (bastile_registry_parent_class)->finalize (obj);
}

static void
bastile_registry_class_init (BastileRegistryClass *klass)
{
	GObjectClass *gobject_class;
	gobject_class = (GObjectClass*)klass;

	bastile_registry_parent_class = g_type_class_peek_parent (klass);
	gobject_class->dispose = bastile_registry_dispose;
	gobject_class->finalize = bastile_registry_finalize;

	g_type_class_add_private (gobject_class, sizeof (BastileRegistryPrivate));
}
...
BastileRegistry*
bastile_registry_get (void)
{
	if (!registry_singleton) {
		registry_singleton = g_object_new (BASTILE_TYPE_REGISTRY, NULL);
		bastile_cleanup_register (cleanup_registry, NULL);
	}
...
void
bastile_registry_register_type (BastileRegistry *registry, GType type, 
                                 const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;

	if (!registry)
		registry = bastile_registry_get ();
	
	g_return_if_fail (BASTILE_IS_REGISTRY (registry));
	g_return_if_fail (type);
	g_return_if_fail (category);

	pv = BASTILE_REGISTRY_GET_PRIVATE (registry);
...
void
bastile_registry_register_object (BastileRegistry *registry, GObject *object, 
                                   const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
...
void
bastile_registry_register_function (BastileRegistry *registry, gpointer func, 
                                     const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
...
GType
bastile_registry_object_type (BastileRegistry *registry, const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
	GType type;
	...
	g_return_val_if_fail (BASTILE_IS_REGISTRY (registry), 0);
	pv = BASTILE_REGISTRY_GET_PRIVATE (registry);
...
GList*
bastile_registry_object_types (BastileRegistry *registry, const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
	GList *types;
	...
	g_return_val_if_fail (BASTILE_IS_REGISTRY (registry), NULL);
	pv = BASTILE_REGISTRY_GET_PRIVATE (registry);
...
GObject*
bastile_registry_object_instance (BastileRegistry *registry, const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
	GObject *object;
...
GList*
bastile_registry_object_instances (BastileRegistry *registry, const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
	GList *objects;
...
gpointer
bastile_registry_lookup_function (BastileRegistry *registry, const gchar *category, ...)
{
	BastileRegistryPrivate *pv;
	va_list cats;
	gpointer func;
