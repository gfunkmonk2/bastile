 ...
 */
void
bastile_util_show_error (GtkWidget *parent, const gchar *heading, const gchar *message)
{
	GtkWidget *dialog;
 ...
 */
void
bastile_util_handle_error (GError* err, const char* desc, ...)
{
    gchar *t = NULL;
    ...
    bastile_util_show_error (NULL, t, err->message ? err->message : "");
    g_free(t);
}    

/**
 * bastile_util_prompt_delete:
 ...
 */
gboolean
bastile_util_prompt_delete (const gchar *text, GtkWidget *parent)
{
...
/**
 * bastile_util_error_domain:
 *
 * Returns: The GError domain for generic bastile errors
 **/
GQuark
bastile_util_error_domain ()
{
    static GQuark q = 0;
    if(q == 0)
        q = g_quark_from_static_string ("bastile-error");
    return q;
}

/** 
 * bastile_util_get_date_string:
 ...
 **/
gchar*
bastile_util_get_date_string (const time_t time)
{
	GDate *created_date;
...
/** 
 * bastile_util_get_display_date_string:
 * @time: Time value to parse
 *
 ...
 **/
gchar*
bastile_util_get_display_date_string (const time_t time)
{
	GDate *created_date;
	gchar *created_string;
...
/**
 * bastile_util_get_text_view_text:
 ...
 */
gchar*
bastile_util_get_text_view_text (GtkTextView *view)
{
...
/**
 * bastile_util_set_text_view_string:
 ...
 */
void
bastile_util_set_text_view_string (GtkTextView *view, GString *string)
{
    GtkTextBuffer *buffer;
 ...
 **/
guchar*
bastile_util_read_to_memory (GInputStream *input, guint *len)
{
	gint size = 128;
	gchar *buffer, *text;
	gsize nread = 0;
 ...
 */
guint
bastile_util_read_data_block (GString *buf, GInputStream *input, 
...
/**
 * bastile_util_memory_input_string:
...
/**
 * bastile_util_memory_output_length:
 * @output: a stream
 *
 ...
 */
gsize
bastile_util_memory_output_length (GMemoryOutputStream *output)
{
	GSeekable *seekable;
	goffset offset, end;
 ...
 **/
gboolean 
bastile_util_print_fd (int fd, const char* s)
{
 ...
 **/
gboolean 
bastile_util_printf_fd (int fd, const char* fmt, ...)
{
    gchar* t;
    va_list ap;
    gboolean ret;
    ...
    ret = bastile_util_print_fd (fd, t);
    g_free (t);
    return ret;
}

/**
 * bastile_util_filename_for_objects:
 ...
 */
gchar*      
bastile_util_filename_for_objects (GList *objects)
{
	...
	if (g_list_length (objects) == 1) {
		g_return_val_if_fail (BASTILE_IS_OBJECT (objects->data), NULL);
		object = BASTILE_OBJECT (objects->data);
		name = bastile_object_get_nickname (object);
		if (name == NULL)
			name = _("Key Data");
	} else {
	...
	filename = g_strconcat (name, BASTILE_EXT_ASC, NULL);
	g_strstrip (filename);
 ...
 **/
const gchar* 
bastile_util_uri_get_last (const gchar* uri)
{
    const gchar* t;
 ...
 **/
const gchar* 
bastile_util_uri_split_last (gchar* uri)
{
    gchar* t;
    
    t = (gchar*)bastile_util_uri_get_last (uri);
    if(t != uri)
        *(t - 1) = 0;
 ...
 **/
gboolean
bastile_util_uri_exists (const gchar* uri)
{
	GFile *file;
	gboolean exists;
 ...
 **/
gchar*
bastile_util_uri_unique (const gchar* uri)
{
    gchar* suffix;
    gchar* prefix;
    gchar* uri_try;
    ...
    if (!bastile_util_uri_exists (uri))
        return g_strdup (uri);
    ...
    for (i = 1; i < 1000; i++) {
        ...
        if (!bastile_util_uri_exists (uri_try))
            break;
 ...
 **/
gchar* 
bastile_util_uri_replace_ext (const gchar *uri, const gchar *ext)
{
    gchar* ret;
    gchar* dot;
    gchar* slash;
 ...
 */
gboolean
bastile_util_uris_package (const gchar* package, const char** uris)
{
    GError* err = NULL;
    gchar *out = NULL;
    gint status;
    ...
    if (!r) {
        bastile_util_handle_error (err, _("Couldn't run file-roller"));
        g_clear_error (&err);
        return FALSE;
    }
    
    if(!(WIFEXITED(status) && WEXITSTATUS(status) == 0)) {
        bastile_util_show_error(NULL, _("Couldn't package files"), 
...
/**
 * bastile_util_detect_mime_type:
 * @mime: The mime string
 *
 * Return the mime type depending on the mime string
 *
 * Returns: BASTILE_PGP, BASTILE_SSH or 0
 */
GQuark
bastile_util_detect_mime_type (const gchar *mime)
{
 ...
 */
GQuark
bastile_util_detect_data_type (const gchar *data, guint length)
{
	gboolean uncertain;
	gchar *mime;
	GQuark type;
	...
	type = bastile_util_detect_mime_type (mime);
	g_free (mime);
...
/**
 * bastile_util_detect_file_type:
 * @uri: The file uri to test for content type
 *
 * Returns: BASTILE_PGP, BASTILE_SSH or 0
 */
GQuark
bastile_util_detect_file_type (const gchar *uri)
{
...
/**
 * bastile_util_write_file_private:
 ...
 */
gboolean
bastile_util_write_file_private (const gchar* filename, const gchar* contents, GError **err)
{
 ...
 */
GtkDialog*
bastile_util_chooser_save_new (const gchar *title, GtkWindow *parent)
{
    GtkDialog *dialog;
 ...
 */
GtkDialog*
bastile_util_chooser_open_new (const gchar *title, GtkWindow *parent)
{
    GtkDialog *dialog;
...
/**
 * bastile_util_chooser_show_key_files:
 ...
 */
void
bastile_util_chooser_show_key_files (GtkDialog *dialog)
{
...
/**
 * bastile_util_chooser_show_archive_files:
 * @dialog: the dialog to add the filter for
 *
 ...
 */
void
bastile_util_chooser_show_archive_files (GtkDialog *dialog)
{
...
/**
 * bastile_util_chooser_set_filename_full:
 ...
 */
void
bastile_util_chooser_set_filename_full (GtkDialog *dialog, GList *objects)
{
    gchar *t = NULL;
    
    if (g_list_length (objects) > 0) {
        t = bastile_util_filename_for_objects (objects);
...
/**
 * bastile_util_chooser_set_filename:
 * @dialog: set the dialog for this
 * @object: The object to use for the filename. #BastileObject
 *
 */
void
bastile_util_chooser_set_filename (GtkDialog *dialog, BastileObject *object)
{
...
/**
 * bastile_util_chooser_save_prompt:
 ...
 */
gchar*      
bastile_util_chooser_save_prompt (GtkDialog *dialog)
{
    GtkWidget* edlg;
    gchar *uri = NULL;
...
/**
 * bastile_util_chooser_open_prompt:
 ...
 */
gchar*      
bastile_util_chooser_open_prompt (GtkDialog *dialog)
{
    gchar *uri = NULL;
 ...
 **/
gboolean
bastile_util_check_suffix (const gchar *path, BastileSuffix suffix)
{
	if (suffix == BASTILE_SIG_SUFFIX)
        return g_str_has_suffix (path, BASTILE_EXT_SIG) || 
               g_str_has_suffix (path, BASTILE_EXT_ASC);
	else
        return g_str_has_suffix (path, BASTILE_EXT_PGP) ||
               g_str_has_suffix (path, BASTILE_EXT_GPG) ||
               g_str_has_suffix (path, BASTILE_EXT_ASC);
}

/**
 * bastile_util_add_suffix:
 ...
 * suffix will be '.pgp' if @suffix is %BASTILE_CRYPT_SUFFIX or '.sig' if 
 * @suffix is %BASTILE_SIG_SUFFIX.
 *
 * Returns: A new path with the suffix appended to @path. NULL if prompt cancelled
 **/
gchar*
bastile_util_add_suffix (const gchar *path, BastileSuffix suffix, const gchar *prompt)
{
    GtkDialog *dialog;
    const gchar *ext;
    gchar *uri;
    ...
    if (suffix == BASTILE_CRYPT_SUFFIX)
        ext = BASTILE_EXT_PGP;
    else
        ext = BASTILE_EXT_SIG;
    ...
    if (prompt && uri && bastile_util_uri_exists (uri)) {
            
        t = g_strdup_printf (prompt, bastile_util_uri_get_last (uri));
        dialog = bastile_util_chooser_save_new (t, NULL);
        g_free (t);
 ...
 **/
gchar*
bastile_util_remove_suffix (const gchar *path, const gchar *prompt)
{
    GtkDialog *dialog;
    gchar *uri;
    gchar *t;
 ...
 */
gchar**
bastile_util_strvec_dup (const gchar** vec)
{
    gint len = 0;
    gchar** ret;
    const gchar** v;
 ...
 */
guint 
bastile_util_strvec_length (const gchar **vec)
{
    guint len = 0;
    while (*(vec++))
        len++;
 ...
 * @k1: the first bastile object
 * @k2: The second bastile object
 *
 * Sorts the bastile objects by their source
 *
 * Returns: if source of k1<k2 it returns -1,
 *          1 will be returned  if k1>k2. If the sources are equal it returns 0
 */
static gint 
sort_objects_by_source (BastileObject *k1, BastileObject *k2)
{
    BastileSource *sk1, *sk2;
    
    g_assert (BASTILE_IS_OBJECT (k1));
    g_assert (BASTILE_IS_OBJECT (k2));
    
    sk1 = bastile_object_get_source (k1);
    sk2 = bastile_object_get_source (k2);
...
/**
 * bastile_util_objects_sort:
 * @objects: #BastileObject #GList to sort
 *
 ...
 */
GList*        
bastile_util_objects_sort (GList *objects)
{
...
/**
 * bastile_util_objects_splice:
 * @objects: A #GList of #BastileObject. Must be sorted
 *
 ...
 */
GList*       
bastile_util_objects_splice (GList *objects)
{
    BastileSource *psk = NULL;
    BastileSource *sk;
    GList *prev = NULL;
    ...
    for ( ; objects; objects = g_list_next (objects)) {
        ...
        g_return_val_if_fail (BASTILE_IS_OBJECT (objects->data), NULL);
        sk = bastile_object_get_source (BASTILE_OBJECT (objects->data));
 ...
 */
gboolean    
bastile_util_string_equals (const gchar *s1, const gchar *s2)
{
    if (!s1 && !s2)
        return TRUE;
    if (!s1 || !s2)
 ...
 */
gchar*
bastile_util_string_up_first (const gchar *orig)
{
    gchar *t, *t2, *ret;
...
/**
 * bastile_util_string_slist_free:
 ...
 */
GSList*
bastile_util_string_slist_free (GSList* list)
{
    GSList *l;
...
/**
 * bastile_util_string_slist_copy:
 ...
 */
GSList*
bastile_util_string_slist_copy (GSList *list)
{
    GSList *l = NULL;
...
/**
 * bastile_util_string_slist_equal:
 ...
 */
gboolean    
bastile_util_string_slist_equal (GSList *l1, GSList *l2)
{
    while (l1 && l2) {
...
/**
 * bastile_util_string_is_whitespace:
 ...
 */
gboolean 
bastile_util_string_is_whitespace (const gchar *text)
{
    g_assert (text);
 ...
 */
gchar*
bastile_util_hex_encode (gconstpointer value, gsize length)
{
	GString *result;
	gsize i;
...
/**
 * bastile_util_determine_popup_menu_position:
 ...
 */
void
bastile_util_determine_popup_menu_position (GtkMenu *menu, int *x, int *y,
 ...
 *
 * Returns: BastileVersion
 **/
BastileVersion
bastile_util_parse_version (const char *version)
{
	BastileVersion ret = 0, tmp = 0;
	int offset = 48;
 ...
 *  - Be usable from libbastile and anything within libbastile
