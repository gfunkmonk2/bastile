...
#include "pgp/bastile-gpgme-data.h"
#include "pgp/bastile-gpg-op.h"
...
#define PRINT(args)  if(!bastile_util_print_fd args) return GPG_E (GPG_ERR_GENERAL)
#define PRINTF(args) if(!bastile_util_printf_fd args) return GPG_E (GPG_ERR_GENERAL)
 ...
 * @length: Length of key, must be within the range of @type specified by #BastileKeyLength
 * @expires: Expiration date of key
 * @err: Catches errors in the params
 * 
 * Tries to generate a new key based on given parameters.
 *
 * Returns: BastileOperation*
 **/
BastileOperation*
bastile_gpgme_key_op_generate (BastileGpgmeSource *psrc, const gchar *name,
                                const gchar *email, const gchar *comment,
                                const gchar *passphrase, const BastileKeyEncType type,
                                const guint length, const time_t expires, gpgme_error_t *err)
{
    BastileGpgmeOperation *pop = NULL;
    ...
    *err = GPG_OK;
    ...
    if (0 != expires)
        expires_date = bastile_util_get_date_string (expires);
    else
    ...
    return pop ? BASTILE_OPERATION (pop) : NULL;
}

/* helper function for deleting @skey */
static gpgme_error_t
op_delete (BastileGpgmeKey *pkey, gboolean secret)
{
	BastileGpgmeSource *psrc;
	gpgme_error_t err;
	gpgme_key_t key;
    
	psrc = BASTILE_GPGME_SOURCE (bastile_object_get_source (BASTILE_OBJECT (pkey)));
	g_return_val_if_fail (psrc && BASTILE_IS_GPGME_SOURCE (psrc), GPG_E (GPG_ERR_INV_KEYRING));
	
	g_object_ref (pkey);
	
	bastile_util_wait_until ((key = bastile_gpgme_key_get_public (pkey)) != NULL);
	
	err = gpgme_op_delete (psrc->gctx, key, secret);
	if (GPG_IS_OK (err))
             bastile_context_remove_object (SCTX_APP (), BASTILE_OBJECT (pkey));
...
gpgme_error_t
bastile_gpgme_key_op_delete (BastileGpgmeKey *pkey)
{
...
gpgme_error_t
bastile_gpgme_key_op_delete_pair (BastileGpgmeKey *pkey)
{
...
typedef gpgme_error_t	(*BastileEditAction) 	(guint			state,
						 gpointer 		data,
						 gint                   fd);
/* Edit transit function */
typedef guint		(*BastileEditTransit)	(guint			current_state,
...
typedef struct
{
	guint 			state;
	gpgme_error_t		err;
	BastileEditAction	action;
	BastileEditTransit	transit;
	gpointer		data;
	
} BastileEditParm;

/* Creates new edit parameters with defaults */
static BastileEditParm*
bastile_edit_parm_new (guint state, BastileEditAction action,
			BastileEditTransit transit, gpointer data)
{
	BastileEditParm *parms;
	
	parms = g_new0 (BastileEditParm, 1);
	parms->state = state;
	parms->err = GPG_OK;
	parms->action = action;
...
static gpgme_error_t
bastile_gpgme_key_op_edit (gpointer data, gpgme_status_code_t status,
                            const gchar *args, int fd)
{
	BastileEditParm *parms = (BastileEditParm*)data;
...
static gpgme_error_t
edit_gpgme_key (gpgme_ctx_t ctx, gpgme_key_t key, BastileEditParm *parms)
{
	...
	out = bastile_gpgme_data_new ();
    
	/* do edit callback, release data */
	gerr = gpgme_op_edit (ctx, key, bastile_gpgme_key_op_edit, parms, out);

	if (gpgme_err_code (gerr) == GPG_ERR_BAD_PASSPHRASE) {
		bastile_util_show_error(NULL, _("Wrong password"), _("This was the third time you entered a wrong password. Please try again."));
	}
	
	bastile_gpgme_data_release (out);
...
static gpgme_error_t
edit_refresh_gpgme_key (gpgme_ctx_t ctx, gpgme_key_t key, BastileEditParm *parms)
{
	gpgme_error_t gerr;
	
	gerr = edit_gpgme_key (ctx, key, parms);
	if (GPG_IS_OK (gerr))
		bastile_gpgme_key_refresh_matching (key);
	
	return gerr;
}

static gpgme_error_t
edit_key (BastileGpgmeKey *pkey, BastileEditParm *parms)
{
	BastileGpgmeSource *psrc;
	gpgme_error_t err;
	gpgme_key_t key;
...
sign_process (gpgme_key_t signed_key, gpgme_key_t signing_key, guint sign_index, 
              BastileSignCheck check, BastileSignOptions options)
{
	BastileEditParm *parms;
	SignParm sign_parm;
	gpgme_ctx_t ctx;
	...
	parms = bastile_edit_parm_new (SIGN_START, sign_action, sign_transit, &sign_parm);
...
gpgme_error_t
bastile_gpgme_key_op_sign_uid (BastileGpgmeUid *uid,  BastileGpgmeKey *signer, 
                                BastileSignCheck check, BastileSignOptions options)
{
	...
	bastile_gpgme_key_get_private (signer);

	g_return_val_if_fail (BASTILE_IS_GPGME_UID (uid), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (signer), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	signing_key = bastile_gpgme_key_get_private (signer);
	g_return_val_if_fail (signing_key, GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	signed_key = bastile_gpgme_uid_get_pubkey (uid);
	g_return_val_if_fail (signing_key, GPG_E (GPG_ERR_INV_VALUE));
	
	sign_index = bastile_gpgme_uid_get_actual_index (uid);
	
	return sign_process (signed_key, signing_key, sign_index, check, options);
}

gpgme_error_t
bastile_gpgme_key_op_sign (BastileGpgmeKey *pkey, BastileGpgmeKey *signer, 
                            BastileSignCheck check, BastileSignOptions options)
{
	gpgme_key_t signing_key;
	gpgme_key_t signed_key;
	...
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (signer), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	signing_key = bastile_gpgme_key_get_private (signer);
	g_return_val_if_fail (signing_key, GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	signed_key = bastile_gpgme_key_get_public (pkey);
...
gpgme_error_t
bastile_gpgme_key_op_change_pass (BastileGpgmeKey *pkey)
{
	BastileEditParm *parms;
	gpgme_error_t err;
	
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));    
	g_return_val_if_fail (bastile_object_get_usage (BASTILE_OBJECT (pkey)) == BASTILE_USAGE_PRIVATE_KEY, GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	parms = bastile_edit_parm_new (PASS_START, edit_pass_action, edit_pass_transit, NULL);
...
/**
 * bastile_gpgme_key_op_set_trust:
 * @pkey: #BastileGpgmeKey whose trust will be changed
 * @trust: New trust value that must be at least #BASTILE_VALIDITY_NEVER.
 * If @pkey is a #BastileKeyPair, then @trust cannot be #BASTILE_VALIDITY_UNKNOWN.
 * If @pkey is not a #BastileKeyPair, then @trust cannot be #BASTILE_VALIDITY_ULTIMATE.
 *
 ...
 **/
gpgme_error_t
bastile_gpgme_key_op_set_trust (BastileGpgmeKey *pkey, BastileValidity trust)
{
	BastileEditParm *parms;
	gint menu_choice;
	...
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	g_return_val_if_fail (trust >= BASTILE_VALIDITY_NEVER, GPG_E (GPG_ERR_INV_VALUE));
	g_return_val_if_fail (bastile_gpgme_key_get_trust (pkey) != trust, GPG_E (GPG_ERR_INV_VALUE));
	
	if (bastile_object_get_usage (BASTILE_OBJECT (pkey)) == BASTILE_USAGE_PRIVATE_KEY)
		g_return_val_if_fail (trust != BASTILE_VALIDITY_UNKNOWN, GPG_E (GPG_ERR_INV_VALUE));
	else
		g_return_val_if_fail (trust != BASTILE_VALIDITY_ULTIMATE, GPG_E (GPG_ERR_INV_VALUE));
	...
	parms = bastile_edit_parm_new (TRUST_START, edit_trust_action,
...
/**
 * bastile_gpgme_key_op_set_disabled:
 * @skey: #BastileKey to change
 ...
 **/
gpgme_error_t
bastile_gpgme_key_op_set_disabled (BastileGpgmeKey *pkey, gboolean disabled)
{
	gchar *command;
	BastileEditParm *parms;
	...
	parms = bastile_edit_parm_new (DISABLE_START, edit_disable_action, edit_disable_transit, command);
	...
	switch (state) {
		...
		case EXPIRE_DATE:
            PRINT ((fd, (parm->expires) ?
				bastile_util_get_date_string (parm->expires) : "0"));
			break;
		case EXPIRE_QUIT:
            PRINT ((fd, QUIT));
			break;
...
gpgme_error_t
bastile_gpgme_key_op_set_expires (BastileGpgmeSubkey *subkey, const time_t expires)
{
	ExpireParm exp_parm;
	BastileEditParm *parms;
	gpgme_key_t key;
	
	g_return_val_if_fail (BASTILE_IS_GPGME_SUBKEY (subkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	g_return_val_if_fail (expires != bastile_pgp_subkey_get_expires (BASTILE_PGP_SUBKEY (subkey)), GPG_E (GPG_ERR_INV_VALUE));
	
	key = bastile_gpgme_subkey_get_pubkey (subkey);
	g_return_val_if_fail (key, GPG_E (GPG_ERR_INV_VALUE));
	
	exp_parm.index = bastile_pgp_subkey_get_index (BASTILE_PGP_SUBKEY (subkey)); 
	exp_parm.expires = expires;
	
	parms = bastile_edit_parm_new (EXPIRE_START, edit_expire_action, edit_expire_transit, &exp_parm);
...
gpgme_error_t
bastile_gpgme_key_op_add_revoker (BastileGpgmeKey *pkey, BastileGpgmeKey *revoker)
{
	BastileEditParm *parms;
	GQuark id;
	
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));    
	g_return_val_if_fail (BASTILE_IS_GPGME_KEY (revoker), GPG_E (GPG_ERR_WRONG_KEY_USAGE));    
	g_return_val_if_fail (bastile_object_get_usage (BASTILE_OBJECT (pkey)) == BASTILE_USAGE_PRIVATE_KEY, GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	g_return_val_if_fail (bastile_object_get_usage (BASTILE_OBJECT (revoker)) == BASTILE_USAGE_PRIVATE_KEY, GPG_E (GPG_ERR_WRONG_KEY_USAGE));

	id = bastile_object_get_id (BASTILE_OBJECT (revoker));
	g_return_val_if_fail (id, GPG_E (GPG_ERR_INV_VALUE));
	
	parms = bastile_edit_parm_new (ADD_REVOKER_START, add_revoker_action,
	                                add_revoker_transit, (gpointer)bastile_pgp_key_calc_rawid (id));
...
gpgme_error_t
bastile_gpgme_key_op_add_uid (BastileGpgmeKey *pkey, const gchar *name, 
                               const gchar *email, const gchar *comment)
{
	BastileEditParm *parms;
	UidParm *uid_parm;
	...
	parms = bastile_edit_parm_new (ADD_UID_START, add_uid_action, add_uid_transit, uid_parm);
...
gpgme_error_t
bastile_gpgme_key_op_add_subkey (BastileGpgmeKey *pkey, const BastileKeyEncType type, 
                                  const guint length, const time_t expires)
{
	BastileEditParm *parms;
	SubkeyParm *key_parm;
	guint real_type;
	BastileKeyTypeTable table;
	gpgme_error_t gerr;
	
    g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), GPG_E (GPG_ERR_WRONG_KEY_USAGE));    
    g_return_val_if_fail (bastile_object_get_usage (BASTILE_OBJECT (pkey)) == BASTILE_USAGE_PRIVATE_KEY, GPG_E (GPG_ERR_WRONG_KEY_USAGE));
	
	gerr = bastile_gpgme_get_keytype_table (&table);
	...
	parms = bastile_edit_parm_new (ADD_KEY_START, add_key_action, add_key_transit, key_parm);
...
gpgme_error_t
bastile_gpgme_key_op_del_subkey (BastileGpgmeSubkey *subkey)
{
	BastileEditParm *parms;
	gpgme_key_t key;
	int index;
	...
	index = bastile_pgp_subkey_get_index (BASTILE_PGP_SUBKEY (subkey));
	parms = bastile_edit_parm_new (DEL_KEY_START, del_key_action,
...
gpgme_error_t
bastile_gpgme_key_op_revoke_subkey (BastileGpgmeSubkey *subkey, BastileRevokeReason reason, 
                                     const gchar *description)
{
	...
	gsubkey = bastile_gpgme_subkey_get_subkey (subkey);
	g_return_val_if_fail (!gsubkey->revoked, GPG_E (GPG_ERR_INV_VALUE));
	
	key = bastile_gpgme_subkey_get_pubkey (subkey);
	g_return_val_if_fail (key, GPG_E (GPG_ERR_INV_VALUE));
	
	rev_parm.index = bastile_pgp_subkey_get_index (BASTILE_PGP_SUBKEY (subkey));
	rev_parm.reason = reason;
	rev_parm.description = description;
	
	parms = bastile_edit_parm_new (REV_SUBKEY_START, rev_subkey_action,
...
gpgme_error_t   
bastile_gpgme_key_op_primary_uid (BastileGpgmeUid *uid)
{
	...
	userid = bastile_gpgme_uid_get_userid (uid);
	g_return_val_if_fail (userid != NULL && !userid->revoked && !userid->invalid, 
	                      GPG_E (GPG_ERR_INV_VALUE));
    
	key = bastile_gpgme_uid_get_pubkey (uid);
	g_return_val_if_fail (key, GPG_E (GPG_ERR_INV_VALUE));
    
	pri_parm.index = bastile_gpgme_uid_get_actual_index (uid);
   
	parms = bastile_edit_parm_new (PRIMARY_START, primary_action,
...
gpgme_error_t   
bastile_gpgme_key_op_del_uid (BastileGpgmeUid *uid)
{
	...
	del_uid_parm.index = bastile_gpgme_uid_get_actual_index (uid);
	
	parms = bastile_edit_parm_new (DEL_UID_START, del_uid_action,
...
/* action helper for adding a photoid to a #BastileKey */
static gpgme_error_t
...
gpgme_error_t 
bastile_gpgme_key_op_photo_add (BastileGpgmeKey *pkey, const gchar *filename)
{
	BastileEditParm *parms;
	...
	key = bastile_gpgme_key_get_public (pkey);
	g_return_val_if_fail (key, GPG_E (GPG_ERR_INV_VALUE));
    
	photoid_add_parm.filename = filename;
	
	parms = bastile_edit_parm_new (PHOTO_ID_ADD_START, photoid_add_action,
...
gpgme_error_t 
bastile_gpgme_key_op_photo_delete (BastileGpgmePhoto *photo)
{
	DelUidParm del_uid_parm;
	BastileEditParm *parms;
	gpgme_key_t key;
 
	g_return_val_if_fail (BASTILE_IS_GPGME_PHOTO (photo), GPG_E (GPG_ERR_WRONG_KEY_USAGE));
      
	key = bastile_gpgme_photo_get_pubkey (photo);
	g_return_val_if_fail (key, GPG_E (GPG_ERR_INV_VALUE));
    
	del_uid_parm.index = bastile_gpgme_photo_get_index (photo);
...
/* action helper for getting a list of photoids attached to a #BastileKey */
static gpgme_error_t
                      ...
                      const gchar *args, gpointer data, gpgme_error_t *err)
{
	...
	case PHOTO_ID_LOAD_OUTPUT_IMAGE:
		...
		if (g_file_test (parm->output_file, G_FILE_TEST_EXISTS)) {
			...
			photo = bastile_gpgme_photo_new (parm->key, NULL, parm->uid);
			...
			if (!pixbuf) {
				pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (), 
				                                   "mate-unknown", 48, 0, NULL);
			}
			    
			bastile_pgp_photo_set_pixbuf (BASTILE_PGP_PHOTO (photo), pixbuf);
...
gpgme_error_t 
bastile_gpgme_key_op_photos_load (BastileGpgmeKey *pkey)
{
	/* Make sure there's enough room for the .jpg extension */
	gchar image_path[]    = "/tmp/bastile-photoid-XXXXXX\0\0\0\0";
	...
	if(fd == -1) { 
		...
		gerr = bastile_gpg_op_num_uids (NULL, keyid, &(photoid_load_parm.num_uids));
		DEBUG_OPERATION (("PhotoIDLoad Number of UIDs %i\n", photoid_load_parm.num_uids));
        
		if (GPG_IS_OK (gerr)) {
            
			setenv ("BASTILE_IMAGE_FILE", image_path, 1);
			oldpath = getenv("PATH");
			...
			parms = bastile_edit_parm_new (PHOTO_ID_LOAD_START, photoid_load_action,
			...
			if (GPG_IS_OK (gerr))
				bastile_pgp_key_set_photos (BASTILE_PGP_KEY (pkey), photoid_load_parm.photos);
		}
		
		bastile_object_list_free (photoid_load_parm.photos);
	}
...
gpgme_error_t   
bastile_gpgme_key_op_photo_primary (BastileGpgmePhoto *photo)
{
	PrimaryParm pri_parm;
	BastileEditParm *parms;
	gpgme_key_t key;
	...
	pri_parm.index = bastile_gpgme_photo_get_index (photo);
