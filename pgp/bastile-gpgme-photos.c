...
prepare_photo_id (GtkWindow *parent, gchar *path, gchar **result, GError **error)
{
    ...
    *result = NULL;
    ...
    if (!format) {
        g_set_error (error, BASTILE_ERROR, -1, 
    ...
    fd = g_file_open_tmp ("bastile-photo.XXXXXX", result, error);
    if (fd == -1) {
        g_object_unref (pixbuf);
        return FALSE;
    }
...
gboolean
bastile_gpgme_photo_add (BastileGpgmeKey *pkey, GtkWindow *parent, const gchar *path)
{
    gchar *filename = NULL;
    gchar *tempfile = NULL;
    GError *error = NULL;
    ...
    g_return_val_if_fail (BASTILE_IS_GPGME_KEY (pkey), FALSE);

    if (NULL == path) {
        chooser = bastile_util_chooser_open_new (_("Choose Photo to Add to Key"), parent);
    ...
    if (!prepare_photo_id (parent, filename, &tempfile, &error)) {
        if (error) {
            bastile_util_handle_error (error, "Couldn't prepare photo");
            g_clear_error (&error);
        }
        return FALSE;
    }
    
    gerr = bastile_gpgme_key_op_photo_add (pkey, tempfile ? tempfile : filename);
    if (!GPG_IS_OK (gerr)) {
        
        /* A special error value set by bastile_key_op_photoid_add to 
           denote an invalid format file */
        if (gerr == GPG_E (GPG_ERR_USER_1)) 
            bastile_util_show_error (NULL, _("Couldn't add photo"), 
                                      _("The file could not be loaded. It may be in an invalid format"));
        
        else
            bastile_gpgme_handle_error (gerr, _("Couldn't add photo"));
...
gboolean
bastile_gpgme_photo_delete (BastileGpgmePhoto *photo, GtkWindow *parent)
{
    gpgme_error_t gerr;
    GtkWidget *dlg;
    gint response; 

    g_return_val_if_fail (BASTILE_IS_GPGME_PHOTO (photo), FALSE);
    ...
    gerr = bastile_gpgme_key_op_photo_delete (photo);
    if (!GPG_IS_OK (gerr)) {
	    bastile_gpgme_handle_error (gerr, _("Couldn't delete photo"));
        return FALSE;
    }
