...
#include "pgp/bastile-gpg-options.h"

#define  GPG_CONF_HEADER    "# FILE CREATED BY BASTILE\n\n"
 ...
 **/
gboolean
bastile_gpg_options_find (const gchar *option, gchar **value, GError **err)
{
    ...
    return bastile_gpg_options_find_vals (options, value, err);
}
 ...
 **/
gboolean
bastile_gpg_options_change (const gchar *option, const gchar *value,
                             GError **err)
{
    const gchar *options[2];
    ...
    return bastile_gpg_options_change_vals (options, (gchar **)&value, err);
}
