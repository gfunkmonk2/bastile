...
void
bastile_gkr_dialog_begin_request (BastileWidget *swidget, gpointer request)
{
	GtkWidget *dialog;
	
	g_return_if_fail (request);
    
	dialog = bastile_widget_get_toplevel (swidget);
	g_return_if_fail (GTK_IS_WIDGET (dialog));
    
	/* Cancel any old operation going on */
	bastile_gkr_dialog_complete_request (swidget, TRUE);
...
void
bastile_gkr_dialog_complete_request (BastileWidget *swidget, gboolean cancel)
{
	GtkWidget *dialog;
	gpointer request;
	...
	if (request && cancel)
		mate_keyring_cancel_request (request);
